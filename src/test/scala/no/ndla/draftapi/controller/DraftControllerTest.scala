...
class DraftControllerTest extends UnitSuite with TestEnvironment with ScalatraFunSuite {
  ...
  test("POST / should return 201 on created") {
    when(writeService.newArticle(any[NewArticle], any[Option[String]], any[Seq[String]], any[Option[Date]], any[Option[Date]]))
  ...
  test("That PATCH /:id returns 403 if access denied") {
    when(writeService.updateArticle(any[Long], any[api.UpdatedArticle], any[Option[String]], any[Seq[String]], any[Option[Date]], any[Option[Date]]))
  ...
  test("That PATCH /:id returns 200 on success") {
    when(writeService.updateArticle(any[Long], any[UpdatedArticle], any[Option[String]], any[Seq[String]], any[Option[Date]], any[Option[Date]]))
