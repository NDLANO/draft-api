...
class ConverterServiceTest extends UnitSuite with TestEnvironment {
  ...
  test("stateTransitionsToApi should return only certain entries if user only has write roles") {
    ...
    res(AWAITING_QUALITY_ASSURANCE.toString).length should be(7)
    res(QUALITY_ASSURED.toString).length should be(3)
    res(QUALITY_ASSURED_DELAYED.toString).length should be(3)
    res(QUEUED_FOR_PUBLISHING.toString).length should be(3)
    res(QUEUED_FOR_PUBLISHING_DELAYED.toString).length should be(3)
  ...
  test("stateTransitionsToApi should return all entries if user is admin") {
    ...
    res(IMPORTED.toString).length should be(1)
    res(DRAFT.toString).length should be(5)
    res(PROPOSAL.toString).length should be(10)
    res(USER_TEST.toString).length should be(7)
    res(AWAITING_QUALITY_ASSURANCE.toString).length should be(9)
    res(QUALITY_ASSURED.toString).length should be(7)
    res(QUALITY_ASSURED_DELAYED.toString).length should be(5)
    res(QUEUED_FOR_PUBLISHING.toString).length should be(5)
    res(QUEUED_FOR_PUBLISHING_DELAYED.toString).length should be(5)
    res(PUBLISHED.toString).length should be(6)
    res(AWAITING_UNPUBLISHING.toString).length should be(6)
    res(UNPUBLISHED.toString).length should be(6)
  }
