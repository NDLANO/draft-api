...
class ConverterServiceTest extends UnitSuite with TestEnvironment {
  ...
  test("toDomainArticle should remove PUBLISHED when merging an UpdatedArticle into an existing") {
    ...
    val Success(res) =
      service.toDomainArticle(existing, TestData.sampleApiUpdateArticle.copy(language = Some("en")), isImported = false)
  ...
  test("toDomainArticle should fail if trying to update language fields without language being set") {
    ...
    val res =
      service.toDomainArticle(TestData.sampleDomainArticle.copy(status = Set()), updatedArticle, isImported = false)
