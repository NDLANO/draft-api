...
import no.ndla.draftapi.model.domain.NdlaSearchException
...
object JestClientFactory {
  def getClient(searchServer: String = DraftApiProperties.SearchServer): NdlaJestClient = {
    DraftApiProperties.RunWithSignedSearchRequests match {
  ...
  private def getSigningClient(searchServer: String): NdlaJestClient = {
    ...
    val awsSigner = new AWSSigner(new DefaultAWSCredentialsProviderChain(), DraftApiProperties.SearchRegion, "es", clock)
