...
import no.ndla.draftapi.DraftApiProperties.{SearchServer, RunWithSignedSearchRequests}
...
object Elastic4sClientFactory {
  ...
  private def getSigningClient(searchServer: String): HttpClient = {
    val elasticSearchUri =
      s"elasticsearch://${searchServer.hostOption.getOrElse("localhost")}:${searchServer.port.getOrElse(80)}?ssl=false"
