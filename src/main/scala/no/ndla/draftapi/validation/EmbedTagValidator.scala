...
import no.ndla.draftapi.model.domain.EmbedTag.EmbedTagAttributeRules
import no.ndla.draftapi.model.domain.{Attributes, EmbedTag, ResourceType}
...
class EmbedTagValidator {
  def validate(fieldName: String, content: String): Seq[ValidationMessage] = {
    val document = HtmlTools.stringToJsoupDocument(content)
  ...
  private def attributesAreLegal(fieldName: String, attributes: Map[String, String]): Option[ValidationMessage] = {
    val legalAttributeKeys = HtmlTools.legalAttributesForTag(resourceHtmlEmbedTag)
