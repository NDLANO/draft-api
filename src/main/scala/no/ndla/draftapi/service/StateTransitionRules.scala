...
trait StateTransitionRules {
  ...
  object StateTransitionRules {
    ...
    val StateTransitions: Set[StateTransition] = Set(
      ...
      (QUALITY_ASSURED_DELAYED       -> QUEUED_FOR_PUBLISHING)         keepStates Set(IMPORTED, USER_TEST, PUBLISHED) require PublishRoles withSideEffect validateArticleApiArticle keepCurrentOnTransition,
      ...
      (QUALITY_ASSURED_DELAYED       -> PUBLISHED)                     keepStates Set(IMPORTED) require DirectPublishRoles withSideEffect publishArticle,
